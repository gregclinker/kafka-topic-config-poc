...
public class AppTest {
    ...
    private ETopicManagerConfig showTopics() throws InterruptedException, ExecutionException, JsonProcessingException {
        ...
        try (AdminClient adminClient = AdminClient.create(TopicManagerJobConfig.getConfig().getKafkaProperties())) {
            ...
            topics.forEach(topic -> {
                ...
                try {
                    ...
                    topicManagerConfig.getTopicConfigs().add(topicConfig);
    ...
    public void test2() throws Exception {
        ETopicManagerConfig topicManagerConfig = new ETopicManagerConfig(this.getClass().getResourceAsStream("/test-topic-config1.yaml"));
        assertNotNull(topicManagerConfig);
        alterTopicConfigs(topicManagerConfig);
        showTopics();

        topicManagerConfig = new ETopicManagerConfig(this.getClass().getResourceAsStream("/test-topic-config2.yaml"));
        assertNotNull(topicManagerConfig);
        alterTopicConfigs(topicManagerConfig);
        showTopics();
    }

    private void alterTopicConfigs(ETopicManagerConfig topicManagerConfig) throws IOException, InterruptedException, ExecutionException {
        try (AdminClient adminClient = AdminClient.create(TopicManagerJobConfig.getConfig().getKafkaProperties())) {
